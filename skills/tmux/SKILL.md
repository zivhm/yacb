...
```bash
SOCKET_DIR="${YACB_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/yacb-tmux-sockets}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/yacb.sock"
SESSION=yacb-python
...
- Use `YACB_TMUX_SOCKET_DIR`.
- Default socket path: `"$YACB_TMUX_SOCKET_DIR/yacb.sock"`.
...
- List sessions on your socket: `scripts/find-sessions.sh -S "$SOCKET"`.
- Scan all sockets: `scripts/find-sessions.sh --all` (uses `YACB_TMUX_SOCKET_DIR`).
...
- Wait for prompts: `scripts/wait-for-text.sh -t session:0.0 -p 'pattern'`.
